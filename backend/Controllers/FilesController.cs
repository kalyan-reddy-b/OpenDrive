...
public class FilesController : BaseController
{
    ...
    {
        if (request == null || string.IsNullOrWhiteSpace(request.TargetEmailOrUsername))
        {
            return BadRequest(new { success = false, message = "Recipient email or username is required." });
        }

        var response = await _fileService.ShareFileAsync(UserId, id, request.TargetEmailOrUsername);
        return response.Success ? Ok(response) : BadRequest(response);
    }

    [HttpGet("shared-with-me")]
    public async Task<IActionResult> GetSharedWithMe([FromQuery] PagedRequest request)
    {
        var response = await _fileService.GetSharedWithMeFilesAsync(UserId, request);
        return Ok(response);
    }

    [HttpDelete("{id}/share/{sharedWithUserId}")]
    public async Task<IActionResult> RemoveShare(Guid id, Guid sharedWithUserId)
    {
        var response = await _fileService.RemoveShareAsync(UserId, id, sharedWithUserId);
        return response.Success ? Ok(response) : BadRequest(response);
    }
}
...
{
    public string TargetEmailOrUsername { get; set; } = string.Empty;
}
