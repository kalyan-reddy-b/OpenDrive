...
export const filesApi = {
  ...
  shareFile: async (id: string, targetEmailOrUsername: string): Promise<ApiResponse<boolean>> => {
    const res = await api.post<ApiResponse<boolean>>(`/Files/${id}/share`, {
      targetEmailOrUsername,
    });
    return res.data;
  },

  /** List files shared with the current user */
  getSharedFiles: async (params: GetFilesParams = {}): Promise<PagedResponse<FileDto>> => {
    const res = await api.get<PagedResponse<FileDto>>('/Files/shared-with-me', { params });
    return res.data;
  },

  /** Revoke sharing for a user */
  removeShare: async (id: string, sharedWithUserId: string): Promise<ApiResponse<boolean>> => {
    const res = await api.delete<ApiResponse<boolean>>(`/Files/${id}/share/${sharedWithUserId}`);
    return res.data;
  },
};
